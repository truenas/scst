...
/*
 * No locks. The caller must hold a tgt_kobj reference, which keeps the tgt
 * memory valid; unlike scst_check_grab_tgtt_ptr() no list lookup is needed,
 * only the check that unregistration has not started yet.
 */
static int scst_check_grab_tgt_ptr(struct scst_tgt *tgt)
{
	int res = 0;
	...
	if (tgt->tgt_unregistering) {
		...
		tgt->tgt_active_sysfs_works_count++;
	}
...
/* No locks */
static void scst_ungrab_tgt_ptr(struct scst_tgt *tgt)
{
	TRACE_ENTRY();
...
static int scst_process_tgt_enable_store(struct scst_tgt *tgt, bool enable)
{
	...
	/* Tgt memory protected by kobject reference */

	TRACE_DBG("tgt %s, enable %d", tgt->tgt_name, enable);

	res = scst_check_grab_tgt_ptr(tgt);
	if (res != 0) {
		PRINT_WARNING("Ignoring stale %s of target %s: unregistration in progress",
			      enable ? "enable" : "disable", tgt->tgt_name);
		/*
		 * Unregistration disables the target anyway, so a concurrent
		 * disable has effectively succeeded; refuse only an enable.
		 */
		if (!enable)
			res = 0;
		goto out_put;
...
static int scst_process_rel_tgt_id_store(struct scst_sysfs_work_item *work)
{
	...
	/* tgt memory protected by kobject_get() */

	TRACE_DBG("Trying to set relative target port id %d",
		(uint16_t)rel_tgt_id);

	/*
	 * is_target_enabled() calls into the target driver, whose per-target
	 * data is freed by tgtt->release() during unregistration, so a stale
	 * queued work must not get past this point (cf.
	 * scst_process_tgt_enable_store()).
	 */
	res = scst_check_grab_tgt_ptr(tgt);
	if (res != 0) {
		PRINT_WARNING("Ignoring stale rel_tgt_id write for target %s: unregistration in progress",
			      tgt->tgt_name);
		goto out_put;
	}
